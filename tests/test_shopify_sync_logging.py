...
class ShopifySyncLoggingTests(unittest.TestCase):
    ...
    def test_sync_products_reconciles_existing_variant_row_before_upsert(self):
        ...
        self.assertEqual(count, 1)
        self.assertTrue(any("INSERT INTO items ( sku, display_sku" in query for query, _ in executed))
        item_insert_index = next(index for index, (query, _params) in enumerate(executed) if "INSERT INTO items ( sku, display_sku" in query)
        child_update_index = next(index for index, (query, _params) in enumerate(executed) if "UPDATE item_location_inventory" in query)
        self.assertLess(item_insert_index, child_update_index)
        copy_query, copy_params = next((q, p) for q, p in executed if "INSERT INTO items ( sku, display_sku" in q)
        self.assertIn("ON CONFLICT (sku) DO NOTHING", copy_query)
        self.assertEqual(copy_params, ("SKU-11", "SKU-11", "__shopify_variant__11"))
